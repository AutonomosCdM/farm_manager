...
class IrrigationDecisionSystem:
    ...
    def __init__(self, region: str = 'Los Ríos'):
        ...
        self.irrigation_model = RandomForestRegressor(n_estimators=100, random_state=42)
        self.scaler = StandardScaler()
        
        # Historial de retroalimentación para entrenamiento del modelo
    ...
    def log_irrigation_feedback(
        ...
        y entrena el modelo de machine learning.
        ...
        try:
            # Convertir características relevantes a vector numérico
            features = [
                ...
                {'dormancia': 0, 'floracion': 1, 'cuaja': 2, 
                 'desarrollo_fruto': 3, 'cosecha': 4}.get(recommendation['crop_stage'], 2),
                ...
                {'Riego Mínimo': 0, 'Riego Moderado': 1, 'Riego Intensivo': 2}
                    .get(recommendation['recommendation'], 1)
            ]
            
            # Mapear resultado real a valor objetivo
            target = 1 if actual_outcome.get('success', False) else 0
            
            # Agregar al conjunto de entrenamiento
            if len(self.feedback_history) > 10:  # Entrenar cuando hay suficientes datos
                X = [entry['features'] for entry in self.feedback_history if 'features' in entry]
                y = [entry['target'] for entry in self.feedback_history if 'target' in entry]
                
                # Escalar características
                X_scaled = self.scaler.fit_transform(X)
                
                # Entrenar modelo
                self.irrigation_model.fit(X_scaled, y)
                
                self.logger.info("Modelo de recomendación de riego actualizado")
        
        except Exception as e:
            self.logger.error(f"Error al procesar retroalimentación: {e}")
    ...
    def validate_recommendation(self, recommendation: Dict[str, Any]) -> Dict[str, Any]:
        ...
        y utiliza el modelo de machine learning para evaluar confiabilidad.
        ...
        # Preparar características para predicción del modelo
        try:
            features = [
                recommendation['current_conditions'].get('temperature', 0),
                recommendation['current_conditions'].get('humidity', 0),
            ...
            ml_prediction = self.irrigation_model.predict_proba(features_scaled)[0][1]
            
            # Validación final
            validation_result = {
                'basic_criteria_met': all(basic_criteria),
                'ml_prediction_probability': ml_prediction,
                'overall_confidence': (recommendation.get('confidence', 0) + ml_prediction) / 2,
                'is_valid': (recommendation.get('confidence', 0) > 0.6) and (ml_prediction > 0.5)
            }
            
            return validation_result
        
        except Exception as e:
            self.logger.error(f"Error en validación de recomendación: {e}")
            return {
                'basic_criteria_met': all(basic_criteria),
