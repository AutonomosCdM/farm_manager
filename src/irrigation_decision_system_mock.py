...
class MockWeatherClient:
    """Cliente meteorológico simulado para pruebas"""
    
    def get_weather_by_region(self, region: str) -> Dict[str, Any]:
        """Retorna datos meteorológicos simulados para una región"""
        return {
            'temperature': 25,
            'humidity': 40,
            'wind_speed': 5,
...
class MockKnowledgeBase:
    """Base de conocimiento simulada para pruebas"""
    
    def query_crops(self, crop_name: str) -> list:
        """Consulta información simulada de cultivos"""
        if crop_name == "Avellano Europeo":
            return [{
                'metadata': {
...
class MockCropCalendar:
    """Calendario de cultivos simulado para pruebas"""
    
    def get_current_stage(self, crop_name: str) -> Dict[str, Any]:
        """Retorna la etapa actual simulada de un cultivo"""
        return {
            'crop': crop_name,
            'stage': 'desarrollo_fruto',
            'description': 'Desarrollo y maduración de frutos',
            'recommended_actions': ['Monitoreo de crecimiento', 'Control de plagas']
        }
...
class IrrigationDecisionSystem:
    ...
    Versión simulada para pruebas sin dependencias externas.
    """
    ...
    def __init__(self, region: str = 'Los Ríos'):
        ...
        """
        self.weather_client = MockWeatherClient()
        self.knowledge_base = MockKnowledgeBase()
        self.crop_calendar = MockCropCalendar()
        self.region = region
